...
## Simplification du prompt image (2026-09-25)

- Un seul prompt `brief_image` v1 (variables `{garment_en}`, `{inspi_texte}`, `{cadrage}`) et un prompt `cadrages` v1 (une ligne `cle : texte` par plan : `porte_miroir`, `porte_sans_miroir` si `texte_visible`, `cintre`, `detail`, variable `{mannequin_desc}`). Ils remplacent `brief_commun`, `brief_garment_en`, `brief_inspi_texte`, `brief_ref_*` et `brief_plan_*`, qui ne sont plus lus (toujours actifs dans la table, sans effet ; `check_fidelite` et `config_pastilles` aussi).
- Le décor vient uniquement de l'inspiration principale (1 image, envoyée en dernier) ; les 2-3 inspirations texte n'apportent que des éléments. Plus aucun décor en dur. Le prompt demande de reproduire fidèlement les défauts visibles du produit.
- Sous-workflow `Tentative image` (`pGvWMH08Go4yGJYp`) : nœuds `Préparer la tentative` et `Assembler la requête image` réécrits ; erreur explicite si aucune inspiration principale. `POST /job/valider` : 400 sans exactement 1 `decor_refs`.
- Front : 1 inspiration principale obligatoire, plus de pastilles (consigne libre seule), plus d'étalonnage ni de « Voir brut ». Le mood n'est plus injecté dans le prompt.
- Nettoyage (2026-09-25) : les 14 prompts obsolètes (`brief_commun`, `brief_garment_en`, `brief_inspi_texte`, `brief_plan_a_plat/cintre/detail/porte_miroir/porte_sans_miroir`, `brief_ref_decor/mannequin/vetement`, `check_fidelite`, `config_pastilles`, `selection_style`) sont supprimés de la table `prompts` (sauvegarde hors dépôt). `Régénérer un plan` n'a plus de logique de pastilles (le webhook ignore `pastilles`), `GET /config` ne renvoie plus `pastilles`, `Proposer un ajustement` ne connaît plus que `texte_fiche`, `brief_image` et `cadrages` (plus de dérive de fidélité), `Sélectionner le style` propose 1 image + 3 textes. Reste inerte mais inoffensif : les colonnes `fidelite_*` de `shots`, `questions` et `mannequin_file_id` de `jobs`, les champs `fidelite_*` renvoyés par `/job-status` et écrits dans `log.json`.
- Retouches après le test « chaussures Valentino » (2026-09-25) : `brief_image` v3 (une seule photo continue, sans collage ni vignette ; produit une seule fois ; rien d'ajouté comme boîte ou étiquette ; variable `{marque}` = marque du formulaire, écrite seulement si une étiquette est visible ; défauts reproduits), `cadrages` v2 (plus de « un tiers de l'image », cause du collage), `texte_fiche` v4 (`garment_en` décrit l'objet seul, indique un article ou une paire, jamais emballage ni décor). Les versions précédentes restent inactives (retour arrière via `/prompts/activer`). `Préparer la tentative` et `Assembler la requête image` gèrent `{marque}` (« not specified » si vide). La v2 inactive de `brief_image` vient d'un test de `/prompts/propose`.
...
## Menu d'accueil, suppression et avis général (2026-09-26)

- **Déploiement** : front publié sur GitHub Pages (`https://jerrylp00-lab.github.io/Vinted/`) par `.github/workflows/pages.yml` à chaque push sur `main` touchant `poc-annonces-n8n/front/`. Manifest PWA (`manifest.webmanifest`) et icônes, sans service worker. Le dossier `poc-annonces/Test_humain/` a été supprimé du dépôt (toujours présent dans l'historique git).
- **Front** : page d'arrivée = grille de cartes (« ＋ Nouvelle annonce » puis les annonces, pastilles Terminé / En cours / À reprendre / Erreur, menu ⋯ pour supprimer). Le bouton « ← Annonces » d'une annonce non terminée propose Sauvegarder pour plus tard (le run reste dans le menu), Jeter (suppression après confirmation) ou Continuer. Fermer sans choisir vaut « sauvegarder ». Sélecteur d'utilisateur retiré (plus de champ `user` envoyé).
- **Nouveaux workflows** : `VFN4 — Supprimer une annonce (webhook)` (`Mohd6FQbdQOz4FW4`, `POST /run/supprimer {run_id}` : dossier Drive à la corbeille, lignes `v4_images` et `v4_runs` supprimées, 404 si inconnue, 409 si génération en cours) et `VFN4 — Avis général sur une annonce (webhook)` (`7YM7UZ5fyACi9Oyi`, `POST /avis {run_id, smiley: mauvais|moyen|bien, commentaire?}`, upsert dans la table `v4_avis` `6p4NuDtfsoiAaE0G`). L'avis est purement informatif (distinct du feedback qui regénère l'image test) et survit à la suppression de l'annonce. `GET /run` renvoie `avis`. `POST /run` n'exige plus `user`.
- **À faire à la main** : vider les lignes de l'ancienne table `feedback` (V3) depuis l'UI n8n. La suppression d'une annonce n'a été testée que sur le cas « annonce inconnue » (404).

## Mannequin homme (2026-09-26)

Le formulaire propose Femme / Homme au-dessus de la description du mannequin ; chaque genre a son défaut, mémorisé à la modification (`config_mannequin` = femme, `config_mannequin_homme` = homme, v1 créée dans `prompts`). `GET /config` renvoie `mannequin` et `mannequin_homme` (et plus `users`) ; `POST /prompt/save` accepte `config_mannequin_homme`. Réglages : deux zones de texte. Le prompt image reste neutre (« the person »), seul le texte du mannequin change.
